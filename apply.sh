...
original_name="python_project_temp_02"
original_urlname="python-project-temp-02"
original_description="Awesome python_project_temp_02 created by kwamedunno" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/python_project_temp_02 "${name}"
...
then
    ...
    echo "then run 'python_project_temp_02' or 'python -m python_project_temp_02'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'python_project_temp_02' or 'python -m python_project_temp_02' or 'poetry run python_project_temp_02'"
fi
